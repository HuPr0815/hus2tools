...
const CITIES = ['北京','上海','广州','深圳','杭州','成都','武汉','南京','重庆','西安','苏州','天津','郑州','长沙','东莞','沈阳','青岛','合肥','佛山','宁波','昆明','大连','厦门','福州','无锡','济南','哈尔滨','温州','南宁','长春','泉州','石家庄','贵阳','南昌','金华','常州','惠州','嘉兴','徐州','南通'];
const STREETS = ['中山路','人民路','解放路','建设路','和平路','光明路','长江路','黄河路','文化路','学院路','科技路','创新路','花园路','幸福路','友谊路','团结路','胜利路','新华路','民主路','自由路'];
const COMPANIES = ['科技有限公司','信息技术有限公司','网络科技有限公司','数据服务有限公司','智能科技有限公司','云计算有限公司','软件开发有限公司','电子商务有限公司','传媒有限公司','咨询有限公司','集团有限公司','实业有限公司','电子科技有限公司','生物科技有限公司','教育科技有限公司','物流有限公司','金融信息服务有限公司','人工智能有限公司','物联网科技有限公司','安全科技有限公司'];
const COMPANY_PREFIXES = ['华','中','天','金','银','恒','瑞','新','盛','达','信','博','创','联','融','通','宏','伟','正','泰'];
...
export function generateAddress(): string {
  return randomItem(CITIES) + '市' + randomItem(STREETS) + (Math.floor(Math.random() * 200) + 1) + '号';
}

export function generateCompany(): string {
  return randomItem(COMPANY_PREFIXES) + randomItem(COMPANY_PREFIXES) + randomItem(COMPANIES);
}

export type DataType = 'phone' | 'idcard' | 'email' | 'name' | 'address' | 'company';
  ...
  for (let i = 0; i < count; i++) {
    ...
    for (const type of types) {
      switch (type) {
        ...
        case 'address': row['地址'] = generateAddress(); break;
        case 'company': row['公司'] = generateCompany(); break;
      }
    }
    results.push(row);
  }
