...
function renderANMFFrame(frameData: Uint8Array, canvasW: number, canvasH: number): Promise<WebPFrame | null> {
...
function generateGridRegions(count: number, layout: string): SpriteRegion[] {
  ...
  const rows = layout === 'horizontal' ? 1 : Math.ceil(count / cols);
